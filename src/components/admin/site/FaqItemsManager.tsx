  ...
  return (
    <div className="grid gap-5 lg:grid-cols-[360px_1fr]">
      ...
      <div className="flex flex-col gap-2">
        {items.length === 0 && (
          <div className="flex flex-col items-center gap-2 rounded-2xl border border-dashed border-dz-primary-200 dark:border-dz-night-border bg-dz-primary-50/30 dark:bg-white/5 p-8 text-center">
            <div className="flex size-11 items-center justify-center rounded-xl bg-white dark:bg-dz-night-card text-dz-primary-400 dark:text-dz-night-faint shadow-xs ring-1 ring-dz-primary-100 dark:ring-dz-night-border"><Plus className="size-5" /></div>
            <p className="text-sm text-dz-primary-500 dark:text-dz-night-muted">هنوز سوالی در این گروه نیست. از فرم کنار، اولین پرسش را اضافه کنید.</p>
          </div>
        )}
        {items.map((r, i) => (
          <div key={r.id} className="rounded-2xl border border-dz-primary-100 dark:border-dz-night-border bg-white dark:bg-dz-night-card p-4 shadow-xs transition-shadow hover:shadow-card">
            <div className="flex items-start justify-between gap-2">
              <div className="min-w-0">
                <div className="flex items-center gap-2">
                  <h3 className="font-medium text-dz-primary-800 dark:text-dz-night-fg">{r.question}</h3>
                  <AdminStatusBadge tone={r.isActive ? "green" : "gray"}>{r.isActive ? "فعال" : "غیرفعال"}</AdminStatusBadge>
                </div>
                <p className="mt-1 line-clamp-2 text-sm text-dz-primary-500 dark:text-dz-night-muted">{htmlToPlainText(r.answer)}</p>
              </div>
              <div className="flex shrink-0 items-center gap-0.5">
                <button type="button" onClick={() => move(r.id, "up")} disabled={i === 0 || pending} title="انتقال به بالا" aria-label="انتقال به بالا" className="focus-ring rounded-lg p-1.5 text-dz-primary-500 dark:text-dz-night-muted transition-colors hover:bg-dz-primary-50 dark:hover:bg-white/5 disabled:opacity-30">
                  <ChevronUp className="size-4" />
                </button>
                <button type="button" onClick={() => move(r.id, "down")} disabled={i === items.length - 1 || pending} title="انتقال به پایین" aria-label="انتقال به پایین" className="focus-ring rounded-lg p-1.5 text-dz-primary-500 dark:text-dz-night-muted transition-colors hover:bg-dz-primary-50 dark:hover:bg-white/5 disabled:opacity-30">
                  <ChevronDown className="size-4" />
                </button>
                <button type="button" onClick={() => startEdit(r)} title="ویرایش" aria-label="ویرایش پرسش" className="focus-ring rounded-lg p-1.5 text-dz-primary-500 dark:text-dz-night-muted transition-colors hover:bg-dz-primary-50 dark:hover:bg-white/5 hover:text-dz-primary-700 dark:hover:text-dz-night-fg">
                  <Pencil className="size-4" />
                </button>
                <button type="button" onClick={() => setConfirmId(r.id)} title="حذف" aria-label="حذف پرسش" className="focus-ring rounded-lg p-1.5 text-dz-error/70 dark:text-dz-error-300 transition-colors hover:bg-dz-error/10 hover:text-dz-error dark:hover:text-dz-error-300">
