...
export default async function FaqsListPage() {
  ...
  const columns: TableColumn<Row>[] = [
    { key: "title", header: "گروه", render: (r) => <span className="font-medium text-dz-a-primary-800 dark:text-dz-a-night-fg">{r.title}</span> },
    { key: "placement", header: "جایگاه", render: (r) => <span className="text-xs text-dz-a-primary-600 dark:text-dz-a-primary-300">{r.placementLabel}</span> },
    ...
    { key: "updatedAt", header: "ویرایش", render: (r) => <span className="text-xs text-dz-a-primary-400 dark:text-dz-a-night-faint">{r.updatedAtLabel}</span> },
    {
      ...
      align: "end",
      render: (r) => (
        <Link href={`/admin/collections/faqs/${r.id}`} className="inline-flex rounded-lg p-1.5 text-dz-a-primary-500 dark:text-dz-a-night-muted hover:bg-dz-a-primary-50 dark:hover:bg-white/5 hover:text-dz-a-primary-700 dark:hover:text-dz-a-night-fg" title="مدیریت">
  ...
  return (
    <div>
      <AdminPageHeader
        ...
        actions={
          <Link href="/admin/collections/faqs/new" className="inline-flex items-center gap-2 rounded-xl bg-dz-a-primary-600 px-4 py-2.5 text-sm font-medium text-white hover:bg-dz-a-primary-700">
      ...
      <AdminDataTable
        ...
        rows={rows}
        getRowId={(r) => r.id}
        empty={<p className="p-10 text-center text-sm text-dz-a-primary-400 dark:text-dz-a-night-faint">هنوز گروهی ثبت نشده.</p>}
      />
    </div>
  );
}
