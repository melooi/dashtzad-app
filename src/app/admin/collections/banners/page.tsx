...
export default async function BannersListPage({
  ...
  const addButton = (
    <Link href="/admin/collections/banners/new" className="inline-flex items-center gap-2 rounded-xl bg-dz-a-primary-600 px-4 py-2.5 text-sm font-medium text-white hover:bg-dz-a-primary-700">
  ...
  const columns: TableColumn<Row>[] = [
    ...
    { key: "window", header: "بازه‌ی نمایش", render: (r) => <span className="text-xs text-dz-a-primary-500 dark:text-dz-a-night-muted">{r.windowLabel}</span> },
    ...
    { key: "updatedAt", header: "ویرایش", render: (r) => <span className="text-xs text-dz-a-primary-400 dark:text-dz-a-night-faint">{r.updatedAtLabel}</span> },
    {
      ...
      align: "end",
      render: (r) => (
        <Link href={`/admin/collections/banners/${r.id}`} className="inline-flex rounded-lg p-1.5 text-dz-a-primary-500 dark:text-dz-a-night-muted hover:bg-dz-a-primary-50 dark:hover:bg-white/5 hover:text-dz-a-primary-700 dark:hover:text-dz-a-night-fg" title="ویرایش">
